...
@app.route("/health")
def health():
    # Keep this endpoint independent from CoinDCX and SQLite so Render can
    # reliably distinguish a healthy web process from upstream API issues.
    return jsonify({"ok": True, "service": "coindcx-scalper"})
...
def api_chart(symbol):
    symbol = symbol.upper()
    pair = f"{config.FUTURES_PAIR_PREFIX}{symbol}{config.FUTURES_QUOTE_SUFFIX}"
    now = time.time()
    ...
    with _chart_cache_lock:
        _chart_cache[symbol] = (now, payload)
...
let signalsPollBusy = false;
let chartPollBusy = false;
...
async function pollSignals() {
  if (signalsPollBusy) return;
  signalsPollBusy = true;
  try {
