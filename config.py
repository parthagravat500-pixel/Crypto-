...
# API docs, which use "B-BTC_USDT" as the example pair for the futures candlesticks endpoint.
...
POLL_INTERVAL_SECONDS = 5           # how often the engine refreshes the RT futures price snapshot and analyzes
# one pair round-robin. With 5 monitored pairs, full indicator analysis for
# each coin refreshes roughly every ~25s, while displayed prices refresh every tick.
